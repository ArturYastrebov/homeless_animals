...
def show_animals_page(response):
    ...
    return render(response, "homeless_animals/show_animals.html", {"animals": animals, "adverts": adverts})
...
def add_animals_upload(request):
    ...
    animal = Animal.objects.create(species=values[0], size=values[1], sex=values[2], age=values[3], lost=values[4])
    advert = Advert.objects.create(animal=animal, city=values[5], coordinates=values[6], description=values[7], inspector=admin)
